(function () {
  ...
  const mapping = window.formContextMapping.find((s) => s.name === 'form1').schema;
    ...
    try {
      ...
      if (aiResponseInStorage) {
        ...
        const missingFields = fieldsArr.filter(
          (ff) => aiResponseInStorage.missing_fields.some((mf) => mf.data_id === ff.data_id) && !ff.fieldValue
        );
        data = {
  ...
  async function sendData(message, fieldData) {
    ...
    try {
      ...
      const response = !mockResponse
        ...
        : // if mocking response
          await Promise.resolve({ ok: true, status: 200, json: async () => window.mockApiResponse });
  ...
  async function populateForm() {
    ...
    if (apiResponse && Array.isArray(apiResponse.filled_fields) && apiResponse.filled_fields.length > 0) {
      /**
   ...
   * enrich with client's form context mapping (if provided)
  ...
  function getFieldsArrFromStorage() {
    ...
    formsDataFromStorage.forEach((form) => {
      form.fields.forEach((field) => {
        // if context found in mapping
        if (mapping) {
          const f = mapping[field.data_id] || {};
          field = { ...field, ...f };
        }
        const idx = fieldsArr.findIndex((item) => item.data_id === field.data_id);
        if (idx !== -1) {
          fieldsArr[idx] = field;
        } else {
  ...
  function helpWithField(fieldId) {
    sendMessage('Assist with filling the field: ' + fieldId);
  }
  ...
  function createChatButton() {
    ...
    button.innerHTML = 'Form Assistant';
    button.className = 'wp-chat-button';
    button.title = 'Form Assistant';
  ...
  window.openLocalPopup = function (aTarget) {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(
        addOrUpdateArray(
          popupsDataInStorage,
          [{ ref: 'myPopup', aAnchor: '', aURL: '', aWidth: '', aHeight: '', aTarget }],
          ['aTarget']
        )
      )
    );
  ...
  function addOrUpdateArray(array, newArray, propertiesToMatch) {
    newArray.forEach((newObj) => {
      const index = array.findIndex((obj) =>
        propertiesToMatch.every((prop) => obj[prop] === newObj[prop] || (obj[prop] == null && newObj[prop] == null))
      );
      if (index !== -1) {
        array[index] = newObj;
      } else {
  ...
  function injectStyles() {
    ...
    style.textContent = `
                ...
                .wp-chat-button {
                    ...
                    background: #003366 url('/aifas-client-scripts/spark_white.svg') no-repeat 25px center;
